...
## P6-F.9.90 Swagger Closure Note — Wednesday Stateful Appointment Context Carryover

Status:

`CLOSED / GREEN LOCALLY / GREEN IN SWAGGER`
...
Ensure that the appointment context generated by a Wednesday appointment request survives between two `/test/message-stateful` calls and allows the unique Wednesday slot to be registered.

Root cause:

The first Swagger message combined appointment intent and an absolute date:

`Quiero agendar una cita para el miércoles 22 de julio de 2026`

The deterministic router correctly classified this message as:

`intent = cita`

The graph correctly resolved:
...
However, `capture_appointment_context_from_state(...)` only captured context when:

`intent = fecha_cita`

Therefore, the first turn persisted the conversational state but did not persist the appointment context.

The second turn recovered `ST_CITA_FRANJA`, but not the date or candidate slot, activating the missing appointment date guard.

Implemented correction:

`capture_appointment_context_from_state(...)` now accepts both valid appointment date-producing intents:
...
* unit coverage for `intent = cita` with an embedded valid date;
* stateful two-call coverage using the real patient repository contract and real graph transition flow.
...
* `nuevo_estado = ST_CITA_PENDIENTE`;
* `persisted_state = ST_CITA_PENDIENTE`;
* the Wednesday date and slot were recovered;
* `appointment_request_decision.should_persist = true`;
* `appointment_request_decision.reason = allowed_hora_cita_ready_for_human_review`;
* `fecha_solicitada = 2026-07-22`;
* `franja_solicitada = 3:00 p. m.–6:00 p. m.`;
...
`SOL-20260716-101032-774734-0091`

Human review side effect:

The controlled request was appended to the configured Google Sheets human review inbox:

* `adapter = google_sheets`;
* `status = appended`.

Known non-blocking response-shape debt:

* top-level `franja_solicitada` remains `null`;
* top-level `dia_semana_solicitado` is not restored in the second turn.

The operational persistence fields are correct in:

* `appointment_request_decision`;
* `appointment_request`.

These cosmetic fields do not block P6-F.9.90 closure and must not interrupt the remaining Swagger validation matrix.
...
* no unrelated Swagger scenario was executed during debugging;
* the Wednesday scenario was retested only after intentional deployment.
...
Resume the paused Swagger matrix from the Saturday availability guard.

Do not repeat already validated scenarios unless a new regression appears.
